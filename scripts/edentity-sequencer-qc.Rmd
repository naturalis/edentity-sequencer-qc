...
## Number of reads per marker.

```{r}
stats_df <- read.csv("../results/illumina-stats.tsv", sep = "\t")
# add pair
stats_df$read_pair <- ifelse(grepl("R1", stats_df$file), "R1", ifelse(grepl("R2", stats_df$file), "R2", NA))
stats_df$sample <- sapply(strsplit(stats_df$file, "_R[12]_"), `[`, 1)
stats_df$marker <- sapply(strsplit(stats_df$file, "_"), `[`, 1)
...
sample_df <- stats_df[stats_df$marker != "Undetermined",]
#sample_df <- sample_df[order(sample_df$num_seqs),]
markers <- with(sample_df, reorder(marker, num_seqs, sum))

markers_df <- sample_df %>%
  group_by(marker, read_pair) %>%
  summarize(total_sequences = sum(num_seqs), .groups = "drop")


ggplot(markers_df, aes(x = reorder(marker,total_sequences),
...
```{r}
write.table(markers_df, "../results/illumina-markers-read-numbers.tsv", row.names = F,sep = "\t")
```
...
illumina_none_empty <- sample_df[sample_df$marker != "Empty",]


# exclude blanks


elements_blank_markers <- c("ITS-Blank", "ITSmix-Blank", "SSU-Blank")
...
average_reads <- data.frame(average_reads = c( mean(illumina_none_empty[illumina_none_empty$read_pair == "R1",]$num_seqs),
...
ggplot(average_reads, aes(x = sequencer,
                                y = average_reads, fill = read_pair)) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_y_continuous(labels = comma)+
  labs(
    title = "Average number of reads per file",
    x = "Sequencer",
    y = "Average reads",
    fill = "Read Pair"
  ) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  geom_text(aes(label = comma(average_reads)), 
            position = position_dodge(width = 0.8), 
            vjust = -0.4, # Adjust the position of the text (above the bars)
            color = "black")
...
| Demultiplexed Reads| 608,699,786 | (- unassigned and adept) |
