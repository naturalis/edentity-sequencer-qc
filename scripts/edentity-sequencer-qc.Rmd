...
fix_csv_commas("elements-1.1.1-qc.txt", "elements-1.1.1-qc-fixed.csv")
fix_csv_commas("illumina-1.1.1-qc.txt", "illumina-1.1.1-qc-fixed.csv")
...
colnames(df) <- c("file", "n_reads", "q30.fraction")
df <- subset(df, select = c("file", "n_reads", "q30.fraction"))

# calculate mean q30 fraction
mean_q30 <- mean(as.numeric(na.omit(df$q30.fraction)))
return(mean_q30 * 100)
}


q30_df <- data.frame(
          Illumina = get_mean_q30("illumina-1.1.1-qc-fixed.csv"),
          Elements = get_mean_q30("elements-1.1.1-qc-fixed.csv"))
q30_df
```


## 1.1.2 Read Length Retension after Q30 Trimming
```{r}
...
length_df <- data.frame(Illumina = get_length_retention(
                                      "illumina-stats.tsv",
                                      "trimmed-illumina-stats.tsv"),
                        
                        Elements = get_length_retention(
                                      "elements-stats.tsv",
                                      "trimmed-elements-stats.tsv"))
...
decay_df <- data.frame(
  Illumina = decay("illumina-1.2.1-homopolymer_decay.csv"),
  Elements = decay("elements-1.2.1-homopolymer_decay.csv")
)
...
demux <- read.csv("illumina-1.2.2-demux.csv")
...
PhiX Coverage Uniformity:
  ...
  - Percentage within ±20% of mean coverage: 0.4330
...
dedup_df <- data.frame(Illumina = average_duplicate_ratio(
                                                "illumina-1.2.4-dedup.csv"),
                       Elements = average_duplicate_ratio(
                                                "elements-1.2.4-dedup.csv"))
dedup_df
```
...
gc_bias <- function(gc_bias_file){
    gc_df <- read.csv("elements-1.2.5.csv")
    return(
     mean(gc_df$Average.Coverage))
  }

gc_bias_coverage = data.frame(
    Elements = gc_bias("elements-1.2.5.csv")
    # illumina coming
)
