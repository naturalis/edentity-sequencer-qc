...
N/B: Illumina used a flow cell with a higher yield capacity (240 GB) compared to the yield capacity used by Elements (160 GB). 

Therefore a more comparable number is the `scaled_proportion`, which is computed by scaling the total number of reads by the yield capacity.  
...
raw_totals <- data.frame(
                            # sum of all reads 
                            total_reads = c(
                                        sum(illumina_stats_df$num_seqs),
                                        sum(elements_stats_df$num_seqs)),
                                        
                            
                            total_files = c( 
                                        dim(illumina_stats_df)[1],
                                        dim(elements_stats_df)[1]),
                                        
                              
                            # average number of reads per file
                            average_reads = c( 
                                            mean(illumina_stats_df$num_seqs),
                                            mean(elements_stats_df$num_seqs)),
                            # pair info
                            yield_capacity = c(240,160)
                            
                            )



raw_totals$scaled_total_reads = raw_totals$total_reads / raw_totals$yield_capacity
raw_totals$scaled_proportion = raw_totals$scaled_total_reads /sum(raw_totals$scaled_total_reads)
raw_totals <- subset(raw_totals, select = -c(scaled_total_reads))
# sequencer info
raw_totals$sequencer <- c("Illumina",  "Elements")
...
illumina_none_empty$scaled_num_seqs <- illumina_none_empty$num_seqs /240
elements_none_empty$sequencer <- "Elements"
elements_none_empty$scaled_num_seqs <- elements_none_empty$num_seqs / 160


# merge both sequencer dfs for visualization
merged_df <- rbind(illumina_none_empty[,c("file", "num_seqs","scaled_num_seqs", "sample", "read_pair", "marker", "sequencer")],
                  elements_none_empty[,c("file", "num_seqs","scaled_num_seqs", "sample", "read_pair", "marker", "sequencer")]
                  )
...
filtered_reads <- data.frame(
                            # sum of all reads 
                            total_reads = c(
                                        sum(illumina_none_empty$num_seqs),
                                        sum(elements_none_empty$num_seqs)),
                                        
                            
                            total_files = c( 
                                        dim(illumina_none_empty)[1],
                                        dim(elements_none_empty)[1]),
                                        
                              
                            # average number of reads per file
                            average_reads = c( 
                                            mean(illumina_none_empty$num_seqs),
                                            mean(elements_none_empty$num_seqs)),
                            # pair info
                            yield_capacity = c(240,160)
                            )

filtered_reads$scaled_total_reads = filtered_reads$total_reads / filtered_reads$yield_capacity
filtered_reads$scaled_proportion = filtered_reads$scaled_total_reads /sum(filtered_reads$scaled_total_reads)
filtered_reads <- subset(filtered_reads, select = -c(scaled_total_reads))

knitr::kable(filtered_reads,align="lr", digits = 2,format.args = list(big.mark = ","))
```
...
merged_df <- merged_df %>%
  mutate(sequencer = reorder(sequencer, scaled_num_seqs, FUN = mean)) 

# Create the violin
ggplot(merged_df, aes(x = sequencer, y = scaled_num_seqs, color = marker)) +
  geom_violin(trim = FALSE, alpha = 0.4) + 
  stat_summary(fun.y = mean, geom = "errorbar",linetype = "solid")+
  scale_y_continuous(labels = comma)+
  labs(y = "sccaled number of reads", x = "Sequencer", 
...
|     - Coverage Range: | 1,254,247 - 14,897,715 | 942,836 - 9,820,668|
...
gc_bias <- function(gc_bias_file){
    ...
    coverage_ratio <- 
      ...
      gc_df_ordered$Average.Coverage[1]
